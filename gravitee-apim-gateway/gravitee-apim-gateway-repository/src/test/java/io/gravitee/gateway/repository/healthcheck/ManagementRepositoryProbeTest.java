...
class ManagementRepositoryProbeTest {
    ...
    @Test
    void should_check_completed_with_healthy_state() {
        cut.check().whenComplete((result, throwable) -> assertThat(result.isHealthy()).isTrue());
    }
    ...
    void should_check_completed_with_unhealthy_state_when_repository_failed() {
        ...
        cut.check().whenComplete((result, throwable) -> assertThat(result.isHealthy()).isFalse());
    }
}
