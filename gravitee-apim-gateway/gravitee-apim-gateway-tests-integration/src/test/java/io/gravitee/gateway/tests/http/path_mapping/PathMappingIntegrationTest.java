 ...
 */
package io.gravitee.gateway.tests.http.path_mapping;
 ...
 */
@GatewayTest
@DeployApi({ "/apis/http/path_mapping/api.json" })
class PathMappingIntegrationTest extends AbstractGatewayTest {
    ...
    void shouldGetPathParamsAsHeaders(HttpClient httpClient) throws InterruptedException {
        wiremock.stubFor(get("/endpoint/123").willReturn(ok("response from backend")));

        httpClient
            .rxRequest(HttpMethod.GET, "/test/123")
        ...
        wiremock.verify(
            1,
            getRequestedFor(urlPathEqualTo("/endpoint/123"))
